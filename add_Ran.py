...
def generate_random_shape():
    # Randomly decide to generate a valid or invalid shape
    if random.choice([True, False]):  # 50% chance of valid/invalid
        return (random.randint(1, 5), random.randint(1, 10))  # Example: (batch_size, features)
    else:
        # Purposefully create an invalid shape (e.g., negative or zero dimensions)
...
def mutate_tensor_params(params):
    ...
    for i in range(len(mutated_params)):
        ...
        if mutated_params[i] < 1:  # Ensure dimensions are at least 1
            mutated_params[i] = -1  # Intentionally setting to an invalid state
