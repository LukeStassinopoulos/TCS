...
def fuzz_test_layer(layer_class, input_shape):
...
def fuzz_test_loss(loss_class, input_shape, target_shape):
    device = device_selection()
...
def main():
    ...
    for layer_class, input_shape in layer_configs:
        fuzz_test_layer(layer_class, input_shape)
    ...
    for loss_class in loss_functions:
        if loss_class == nn.CrossEntropyLoss:
            fuzz_test_loss(loss_class, (1, num_classes), (1,))  # Input for CrossEntropy
        else:
            fuzz_test_loss(loss_class, (1, num_classes), (1, num_classes))  # Input for MSELoss
