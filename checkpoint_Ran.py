...
def mutate_tensor_params(shape):
    ...
    for i in range(len(mutated_shape)):
        mutation = random.choice([-1, 0, random.randint(1, 5)])
        mutated_shape[i] += mutation
        if mutated_shape[i] < 1:  # Ensure shape dimensions are valid
            mutated_shape[i] = 1
...
def run_checkpoint_test_valid(shape):
    try:
        for i in range(10):
...
shape = (2, 2)
run_checkpoint_test_valid(shape)
